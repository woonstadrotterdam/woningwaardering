"""Tests voor de maximering van extra keukenvoorzieningen."""
...
def test_kort_aanrecht_verhoogt_max_niet():
    assert _waardeer_keuken_punten(_keuken_json([1500, 600], 8)) == 8.0


@pytest.mark.filterwarnings("ignore::UserWarning")
def test_twee_geldige_aanrechten_som_basispunten():
    assert _waardeer_keuken_punten(_keuken_json([1000, 1000], 8)) == 16.0


@pytest.mark.filterwarnings("ignore::UserWarning")
def test_beleidsboek_voorbeeld_1600mm():
    eenheid_json = """{
      "id": "keuken_1600mm",
      "ruimten": [{
        "id": "keuken",
