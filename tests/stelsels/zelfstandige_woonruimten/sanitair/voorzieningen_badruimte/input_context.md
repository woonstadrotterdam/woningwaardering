# voorzieningen_badruimte

## Doel

Test extra sanitaire voorzieningen in een badruimte met wastafel en bad: basis 7 punten (bad) + 1 (wastafel) + 5 extra = **13 punten**, met maximeringen op kastruimte en bad/douche.

## Beleidsbron

- Implementatietoelichting: [§2.6.2 Extra sanitaire voorzieningen](../../../../../docs/implementatietoelichtingen/zelfstandige-woonruimten.md#262-punten-voor-extra-sanitaire-voorzieningen)
- Beleidsboek (quote): "Het aantal punten voor extra voorzieningen kan niet meer zijn dan het totaal aantal punten voor de douche, het bad en/of bad/douche gezamenlijk."

## Handmatige berekening

| Categorie                   | Punten    |
| --------------------------- | --------- |
| Wastafel                    | 1,00      |
| Bad                         | 6,00      |
| Bubbelfunctie               | 1,50      |
| Douchewand                  | 1,25      |
| Handdoekenradiator          | 0,75      |
| Ingebouwd kastje            | 1,00      |
| Kastruimte (max 0,75)       | 0,75      |
| Stopcontact                 | 0,25      |
| Éénhandsmengkraan           | 0,25      |
| Thermostatische mengkraan   | 0,50      |
| Max verdubbeling bad/douche | −0,25     |
| **Totaal**                  | **13,00** |
