# 81065000089

## Doel

Ketentest voor het volledige waarderingsstelsel zelfstandige woonruimte op basis van een VERA-export van eenheid **81065000089** (eengezinswoning bouwjaar 1921).
Zolder met vlizotrap; minpunten buitenruimten (−5) door ontbrekende privé-buitenruimte.

## Beleidsbron

- Implementatietoelichting: [§2.2.2.3 Zolderruimte zonder vaste trap](../../../../../docs/implementatietoelichtingen/zelfstandige-woonruimten.md#2223-zolderruimte-zonder-vaste-trap); [§2.8.6 Minpunten bij geen buitenruimte](../../../../../docs/implementatietoelichtingen/zelfstandige-woonruimten.md#285-minpunten-bij-geen-enkele-buitenruimte)
- Beleidsboek (quote): "Als een zolderruimte niet voldoet aan de vereisten voor waardering als een 'vertrek', maar wel als overige ruimte kan worden aangemerkt en er is geen vaste trap naar de zolder, dan worden er 5 punten afgetrokken van de waarde die aan het vloeroppervlak wordt toegekend."

## Handmatige berekening

Verwacht totaal **144 punten** (som van alle rubrieken in `output.json`).

| Rubriek                                                         | Punten  |
| --------------------------------------------------------------- | ------- |
| Oppervlakte van vertrekken                                      | 58      |
| Oppervlakte van overige ruimten                                 | 4.75    |
| Verkoeling en verwarming                                        | 11      |
| Buitenruimten                                                   | -5      |
| Energieprestatie                                                | 34      |
| Keuken                                                          | 7       |
| Sanitair                                                        | 4       |
| Gemeenschappelijke parkeerruimten                               | 0       |
| Gemeenschappelijke vertrekken, overige ruimten en voorzieningen | 0       |
| Punten voor de WOZ-waarde                                       | 30.25   |
| Bijzondere voorzieningen                                        | 0       |
| Prijsopslag monumenten en nieuwbouw                             | 0       |
| **Totaal**                                                      | **144** |
