...
class BijzondereVoorzieningenZorgwoning(Stelselgroep):
    def __init__(
        self,
        ...
        self.stelselgroep = (
            Woningwaarderingstelselgroep.bijzondere_voorzieningen_zorgwoning
        )
        super().__init__(
    ...
    def bereken(
        ...
        woningwaardering_groep = WoningwaarderingResultatenWoningwaarderingGroep(
            criteriumGroep=WoningwaarderingResultatenWoningwaarderingCriteriumGroep(
                ...
                stelselgroep=Woningwaarderingstelselgroep.bijzondere_voorzieningen_zorgwoning.value,
            )
        )
        ...
        if (
            ...
            if not woningwaardering_resultaat or not woningwaardering_resultaat.groepen:
                ...
                woningwaardering_resultaat = OnzelfstandigeWoonruimten(
                    ...
                    eenheid, negeer_stelselgroep=BijzondereVoorzieningenZorgwoning
                )
            ...
            puntentotaal = utils.rond_af(
                sum(
                    ...
                    if (
                        ...
                        not in [
                            Woningwaarderingstelselgroep.bijzondere_voorzieningen_zorgwoning.code,
                            Woningwaarderingstelselgroep.aftrekpunten.code,
                        ]
                    )
                ),
                0,
            ...
            logger.info(
                f"Eenheid {eenheid.id} is een zorgwoning en wordt gewaardeerd met een verhoging van {verhoging} punten voor stelselgroep {self.stelselgroep.naam}"
            )
        ...
        if any(
            ...
            logger.info(
                f"Eenheid {eenheid.id} heeft een aanbelfunctie met video en audioverbinding en wordt met 0,25 punt gewaardeerd voor stelselgroep {self.stelselgroep.naam}"
            )
        ...
        if aantal_laadpalen > 0:
            ...
            logger.info(
                f"Eenheid {eenheid.id} heeft {aantal_laadpalen} {'laadpaal' if aantal_laadpalen == 1 else 'laadpalen'} en wordt met {punten_laadpalen} punten gewaardeerd voor stelselgroep {self.stelselgroep.naam}"
            )
...
if __name__ == "__main__":  # pragma: no cover
    ...
    stelselgroep = BijzondereVoorzieningenZorgwoning(
