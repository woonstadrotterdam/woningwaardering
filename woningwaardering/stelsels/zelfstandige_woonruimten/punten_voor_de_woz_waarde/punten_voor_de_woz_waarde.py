...
class PuntenVoorDeWozWaarde(Stelselgroep):
    ...
    def _corrigeer_woz_punten(
        ...
        woz_punten_onafgerond: Decimal,
    ) -> None:
        """
        Past WOZ-correcties toe: nieuwbouwminimum (§2.11.5) en daarna de 33%-cap (§2.11.7).
        ...
        Args:
            ...
            woningwaardering_resultaat (WoningwaarderingResultatenWoningwaarderingResultaat): Woningwaarderingresultaten.
            woz_punten_onafgerond (Decimal): Som van onafgeronde onderdelen I en II.
        """

        woz_punten = woz_punten_onafgerond
    ...
    def _cap_punten(
        ...
        """
        Berekent de 33%-cap op de WOZ-punten.

        De 187-drempel en de COROP-uitzondering toetst de aanroeper. Deze methode
        beperkt alleen het WOZ-aandeel tot 33% van het totaal en rondt bij toepassing
        naar beneden af op hele punten.

        Args:
            woz_punten (Decimal): Punten voor de stelselgroep WOZ-waarde (I+II).
            overige_punten (Decimal): Som van de overige rubriektotalen (al op kwartpunten).

        Returns:
            Decimal | None: Correctiepunten, of None als de cap het aantal niet verlaagt.
        """
        ...
        return correctiepunten + afrondingscorrectie
    ...
    def _is_corop_regel_van_toepassing(
        ...
        if corop_gebied["code"] not in ["23", "17"]:
            return False

        if eenheid.gebruiksoppervlakte is None:
            warnings.warn(
        ...
        logger.info(
            f"Eenheid ({eenheid.id}): COROP-regels van toepassing - Bouwjaar: {eenheid.bouwjaar}, Gebruiksoppervlakte: {eenheid.gebruiksoppervlakte}m², COROP-gebied: {corop_gebied['naam']} ({corop_gebied['code']})"
        )
...
if __name__ == "__main__":  # pragma: no cover
    with DevelopmentContext(
        instance=PuntenVoorDeWozWaarde(peildatum=date(2026, 7, 1)),
